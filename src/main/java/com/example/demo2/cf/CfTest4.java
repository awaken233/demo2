...
public class CfTest4 {
    ...
    public static <R> R getFastFail(Collection<?> list, Callable<R> callback)
        throws ExecutionException, InterruptedException {
        List<Callable<R>> callableList = list.stream().map(r -> {
            try {
                return callback;
            } catch (Exception e) {
        ...
        List<Future<R>> futures = callableList.stream().map(EXECUTOR::submit)
            .collect(Collectors.toList());
        // 判断 futures 任意一个 isDone
        Future<R> fastFailFuture = getFirstCompleteFuture(futures);
